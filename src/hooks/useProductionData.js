    ...
    try {
      await Promise.all(ALL_KEYS.map(async (k) => {
        try {
          ...
          console.error(`Error fetching ${k}:`, e);
          newData[k] = [];
          newStatus[k] = 'fail';
        }
      }));
      ...
      // Ambil rentang tanggal dengan validasi keamanan
      const allTimestamps = [];
      PROD_KEYS.forEach(k => {
        const cfg = SHEETS[k];
        if (cfg && cfg.i && newData[k]) {
          newData[k].forEach(r => {
            const rawD = r[cfg.i.date];
            if (rawD) {
              const d = parseDateVal(rawD);
      ...
      console.error('Fatal load error:', globalErr);
    } finally {
      // Pastikan loading selalu selesai
      setLoading(false);
    }
  }, []);
